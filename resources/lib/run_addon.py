...
def list_addons(argv):
      #info directory
      addDir('[COLOR blue][B]%s[/B][/COLOR]' % (translate(30001)),'None',None,xbmcaddon.Addon().getAddonInfo('icon'),argv)
      ...
      for individual_addon in listofaddons:
            ...
            if os.path.exists(addon_xml_path):
                  ...
                  if re.search(SERVICE_DISABLED,xml_content):
                        ...
                        addDir('[B][COLOR gold]%s[/B] (off)[/COLOR]' % (individual_addon),path_to_addon,1,addon_icon_path,argv)
                  elif re.search('point="xbmc.service"',xml_content):
                        #addon with service on
                        addDir('%s (on)' % (individual_addon),path_to_addon,1,addon_icon_path,argv)
                  else:
...
def addDir(name,path,mode,iconimage,argv):
      ...
      return xbmcplugin.addDirectoryItem(handle=int(argv[1]),url="%s?path=%s&mode=%s&name=%s" % (argv[0],urllib.parse.quote_plus(path),mode,urllib.parse.quote_plus(name)),listitem=listItem,isFolder=False)
